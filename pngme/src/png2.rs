...
impl Png{
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk>{
        ...
        for i in 0..chunks.len(){
            ...
            if current_chunk.chunk_type().bytes() == chunk.bytes(){
    ...
    fn as_bytes(&self) -> Vec<u8>{
        ...
        for i in 0..chunks.len(){
            let current_chunk = &chunks[i];
            bytes.append(&mut current_chunk.as_bytes());
        }
        return bytes
    }
}
...
impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut string_bytes = String::from("");
        let chunks = &self.chunks;
        for i in 0..chunks.len(){
            let current_chunk = &chunks[i];
            string_bytes.push_str(&format!("{}", current_chunk));
        }
        write!(f, "{}", string_bytes)
    }
}
...
impl TryFrom<&[u8]> for Png{
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let header_bytes: &[u8; 8] = &Png::STANDARD_HEADER;
        ...
        while start < end {
            ...
            //skipping 4 for length, 4 for chunkType, 4 for crc so total 12
            let mut current_chunk_end = start + length as usize + 12;
            let current_chunk = &remaining_bytes[start..current_chunk_end];
            match Chunk::try_from(current_chunk){
